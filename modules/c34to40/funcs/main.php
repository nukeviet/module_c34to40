    ...
    if ($dh) {
        while (($file = readdir($dh)) !== false) {
            if (preg_match('/^([a-zA-Z0-9\-\_\/\.]+)\.php$/', $file)) {
                ...
                $output_data = str_replace('new download(', 'new NukeViet\Files\Download(', $output_data);
                $output_data = str_replace('new image(', 'new NukeViet\Files\Image(', $output_data);
                $output_data = str_replace('new upload(', 'new NukeViet\Files\Upload(', $output_data);
                $output_data = str_replace('new UrlGetContents(', 'new NukeViet\Client\UrlGetContents(', $output_data);
                $output_data = str_replace('new Diagnostic(', 'new NukeViet\Client\Diagnostic(', $output_data);
                $output_data = str_replace('new Gfonts(', 'new NukeViet\Client\Gfonts(', $output_data);
                $output_data = str_replace('new NV_Http(', 'new NukeViet\Http\Http(', $output_data);
                $output_data = str_replace('new NVftp(', 'new NukeViet\Ftp\Ftp(', $output_data);
                ...
                if (preg_match("/^\<\?php[\s\t\r\n]+\/\*\*[\s\t\r\n]+([^\#]+)[\s\t\r\n]+\*\/[\s\t\r\n]+/", $output_data, $m)) {
                    ...
                    $new = str_replace("2009 VINADES", "2016 VINADES", $new);
                    $new = str_replace("2010 VINADES", "2016 VINADES", $new);
                    $new = str_replace("2011 VINADES", "2016 VINADES", $new);
                    $new = str_replace("2012 VINADES", "2016 VINADES", $new);
                    $new = str_replace("2013 VINADES", "2016 VINADES", $new);
